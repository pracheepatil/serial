...
static SerialPort_t* s_devices[MAX_SERIAL_PORTS];
static status_t __updateBaud(SerialPort_t* device, EnSerialBaud_t baud)
{
	return SERIAL_STATUS_NOT_IMPLEMENTED;
}
...
SerialPort_t* SerialPort_new(SerialPortConfig_t* config, status_t* result)
{
	...
	*result = sanity_check(config);
	...
	SerialPort_t* device = (SerialPort_t*)malloc(sizeof(SerialPort_t));
	...
	switch(config->baud)
	{
		...
		UBRRH = (BAUD_9600 >> 8);
		UBRRL = BAUD_9600;
		device->baud = enSerialBaud_9600;
		break;
		case 1:
		UBRRH = (BAUD_19200 >> 8);
		UBRRL = BAUD_19200;
		device->baud =enSerialBaud_19200;
		break;
		case 2:
		UBRRH = (BAUD_38400 >> 8);
		UBRRL = BAUD_38400;
		device->baud = enSerialBaud_38400;
		break;
		case 3:
		UBRRH = (BAUD_57600 >> 8);
		UBRRL = BAUD_57600;
		device->baud = enSerialBaud_57600;
		break;
		case 4:
		UBRRH = (BAUD_115200 >> 8);
		UBRRL = BAUD_115200;
		device->baud = enSerialBaud_115200;
		break;
		default:
